...
public class MapActor extends AbstractActor {
    ...
    List<String> STOP_WORD_LIST = Arrays.asList(STOP_WORDS);

    private ActorRef actorRef = null;

    public MapActor(ActorRef actorRef) {
    ...
    public Receive createReceive() {
        return receiveBuilder()
                .match(String.class, m -> {
                    List<Result> list = evaluateExpression(m);
                    actorRef.tell(list, self());
                })
                .build();
    }
    ...
    private List<Result> evaluateExpression(String line) {
        ...
        while (parser.hasMoreTokens()) {
            ...
            if (isAlpha(word) && !STOP_WORD_LIST.contains(word)) {
